...
- **Jobs Page UX Improvements**: Fixed critical navigation and scroll position issues. When users click a job, the navbar no longer updates unnecessarily and the job list maintains its scroll position. Implemented `window.history.replaceState()` instead of router navigation to prevent scroll resets and added caching controls to prevent unnecessary navbar re-renders.
...
- **Comprehensive Dark Mode Implementation**: Completed comprehensive dark mode support throughout entire application. Removed ALL hardcoded colors (text-gray-*, bg-gray-*, border-gray-* classes) and replaced with theme-aware CSS variables (text-foreground, text-muted-foreground, bg-background, bg-muted, border-border, border-input). Updated all skeleton components to use proper theme variables. Fixed status colors (green, red, yellow) with dark mode variants. Enhanced ThemeProvider with localStorage persistence and automatic theme switching. All 100+ components now properly support light/dark themes with consistent visual appearance.
- **Enhanced Automatic Token Refresh**: Improved token refresh functionality to prevent user logouts when access tokens expire. The system now automatically detects 401 errors, refreshes the access token using the refresh token, and seamlessly retries the original API request. Enhanced error handling with better logging for debugging token issues. FormData requests are properly handled during token refresh retries. Users only get logged out if the refresh token itself is expired or invalid, providing a much smoother authentication experience.
